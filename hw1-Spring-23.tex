...
$\ag{p \land q}$ implies $p \land q$ holds in all successors of $s$ including $s$ itself. As both $p$ and $q$ holds in state $s$, which immediately implies $p\ \Rightarrow\ \af{q}$ true at that state. The Kripke structure satisfying $\ag{p \land q}$ is provided below. 
